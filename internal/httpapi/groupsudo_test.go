...
func callGroupSudo(t *testing.T, s *Server, h http.HandlerFunc, method, group, body string) *httptest.ResponseRecorder {
	t.Helper()
	r := httptest.NewRequest(method, "/api/admin/groups/"+group+"/sudo", strings.NewReader(body))
	r.SetPathValue("name", group)
 ...
 * verildiğini söylemiyor; grubun sudo'su bu üründe erişimin kendisi kadar
 * ağır bir karar.
 */
func TestGroupSudoRuleIsWrittenReadAndAudited(t *testing.T) {
	s, db := dbServer(t)
	if _, err := db.CreateGroup(t.Context(), "dba"); err != nil {
		t.Fatal(err)
	}

	body := `{"commands":[{"path":"/usr/sbin/nginx","args":["-s","reload"]}]}`
	if w := callGroupSudo(t, s, s.adminSetGroupSudo, http.MethodPut, "dba", body); w.Code != http.StatusOK {
		t.Fatalf("yazma: %d %s", w.Code, w.Body.String())
	}

	w := callGroupSudo(t, s, s.adminGroupSudo, http.MethodGet, "dba", "")
 ...
 */
func TestGroupSudoRefusalSaysWhichCommandAndWhy(t *testing.T) {
	s, db := dbServer(t)
	...
	w := callGroupSudo(t, s, s.adminSetGroupSudo, http.MethodPut, "ops", escape)
	...
	if w := callGroupSudo(t, s, s.adminGroupSudo, http.MethodGet, "ops", ""); w.Code != http.StatusNotFound {
	...
	w = callGroupSudo(t, s, s.adminSetGroupSudo, http.MethodPut, "ops", wild)
	...
	if w := callGroupSudo(t, s, s.adminSetGroupSudo, http.MethodPut, "ops", okBody); w.Code != http.StatusOK {
	...
	w = callGroupSudo(t, s, s.adminDeleteGroupSudo, http.MethodDelete, "ops", "")
	...
	if w := callGroupSudo(t, s, s.adminDeleteGroupSudo, http.MethodDelete, "ops", ""); w.Code != http.StatusNotFound {
		t.Errorf("ikinci silme: %d", w.Code)
	}
	if w := callGroupSudo(t, s, s.adminSetGroupSudo, http.MethodPut, "yokrol", okBody); w.Code != http.StatusNotFound {
		t.Errorf("olmayan grup: %d", w.Code)
	}
}
 ...
 */
func TestGroupListCarriesTheSudoRule(t *testing.T) {
	s, db := dbServer(t)
	for _, name := range []string{"dba", "kuralsiz"} {
		if _, err := db.CreateGroup(t.Context(), name); err != nil {
			t.Fatal(err)
		}
	}
	...
	if seen["dba"] == nil || len(seen["dba"].Commands) != 2 ||
		...
		t.Errorf("kurallı grup: %+v", seen["dba"])
	}
	if _, listed := seen["kuralsiz"]; !listed {
		t.Fatal("kuralsız grup listede yok")
	}
	if seen["kuralsiz"] != nil {
		t.Errorf("kuralsız grup kural taşıyor: %+v", seen["kuralsiz"])
	}
}
 ...
 */
func TestGroupSudoEndpointsAreBehindTheAdminAndSameOriginGates(t *testing.T) {
	db := migratedStore(t)
	...
	call := func(method, site string) int {
		t.Helper()
		r := httptest.NewRequest(method, "/api/admin/groups/dba/sudo",
 ...
 */
func TestGroupSudoMarksWhichCommandIsTheWayOut(t *testing.T) {
	s, db := dbServer(t)
	if _, err := db.CreateGroup(t.Context(), "ops"); err != nil {
		t.Fatal(err)
	}
	...
	if w := callGroupSudo(t, s, s.adminSetGroupSudo, http.MethodPut, "ops", body); w.Code != http.StatusOK {
		t.Fatalf("yazma: %d %s", w.Code, w.Body.String())
	}

	w := callGroupSudo(t, s, s.adminGroupSudo, http.MethodGet, "ops", "")
	var got sudoRuleView
