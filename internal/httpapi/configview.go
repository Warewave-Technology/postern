...
var shownConfig = map[string]string{
	...
	"sync.grace":                               "how long someone may be missing before roles are revoked",
	"sync.timeout":                             "how long one directory query may take",
	"sync.max_zero_fraction":                   "blast radius: share of users that may drop to no role in one run",
	...
	"sync.max_revoke_per_run":                  "roles one run may revoke",
